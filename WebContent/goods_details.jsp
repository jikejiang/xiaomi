<%@ page language="java" import="java.util.*,com.mycls.ssm.entity.Good" pageEncoding="UTF-8"%>
...
<html>
  ...
  <head>
	...
	<div class="xzbb ml20 mt10 colorc" id="{{goodColor}}" >
				<div class="banben">
					<a>
					    <input type="radio" id="color{{count2}}" name="color" value="{{goodColor}}" onclick="changeclo()" />
	...
	<script type="text/javascript">
		...
		function check(){
			...
			var goodname = $("input[name='good_name']")[1].value;
			var type = $("input[name='type']").val();
			var price = $("input[name='type']").siblings()[0].children[1].innerText;
			
			$.ajax({
				
				url : "GoodServlet",
				type:"post",
				data:{"color":color,"goodname":goodname,"type":type,"price":price,"operate":"typeforcolor"},
				dataType:"json",
				success:function(data){
					$(".colors").html("");
					for(var i=0;i<data.length;i++){
						var html = $("#moban").html();
						var rphtml = html.replace(/{{count2}}/g,i).replace(/{{goodColor}}/g,data[i].goodColor);
						$(".colors").append(rphtml);
					}
				}
			})
		}
...
<form action="GoodServlet?operate=buy" method="post" name = "form"> 
 <!--  <input type="hidden" id="operate" name="operate" value="buy"/> -->
  <input id="goodname" type="hidden" name="good_name" value="<%=goods.get(0).getGoodName() %>"/>
	<div class="xiangqing">
		<div class="neirong w">
